...
Vue.mixin({
  activated() {
    ...
    if (this.$route.meta && this.$route.meta.title)
      ...
      console.warn(
        "您还未配置该路由的Tab页面显示的标题！{meta:{tile:'页面名称'}}"
      );
    }
  ...
  if (component.$vnode && component.$vnode.data.keepAlive) {
    if (
      ...
      if (component.$vnode.componentOptions) {
        var key =
          component.$vnode.key == null
            ? component.$vnode.componentOptions.Ctor.cid +
              (component.$vnode.componentOptions.tag
                ? `::${component.$vnode.componentOptions.tag}`
                : "")
            : component.$vnode.key;
...
export default {
  ...
  mutations: {
    ActiveTab(state, index) {
      ...
      if (state.Tabs[index].routerparams)
        routerconfig.params = state.Tabs[index].routerparams;
